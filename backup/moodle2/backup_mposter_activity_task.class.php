...
/**
 * Provides the backup_mposter_activity_task class.
 *
 * @package     mod_mposter
 * @category    backup
...
require_once($CFG->dirroot.'/mod/mposter/backup/moodle2/backup_mposter_stepslib.php');

/**
 * Defines the settings and steps to perform a backup of the mposter.
 *
 * @copyright 2015 David Mudrak <david@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_mposter_activity_task extends backup_activity_task {

    /**
     * Defines activity specific settings to be added to the common ones.
     *
     * The mposter module does not define own settings.
     *
    ...
    protected function define_my_steps() {
        $this->add_step(new backup_mposter_activity_structure_step('mposter_structure', 'mposter.xml'));
    }
     ...
     * the course by URLs like http://my.moodle.site/mod/mposter/view.php?id=42
    ...
    static public function encode_content_links($content) {
        ...
        // Link to the list of mposters.
        $search = '/('.$base.'\/mod\/mposter\/index.php\?id\=)([0-9]+)/';
        $content = preg_replace($search, '$@POSTERINDEX*$2@$', $content);

        // Link to a mposter view by cmid.
        $search = '/('.$base.'\/mod\/mposter\/view.php\?id\=)([0-9]+)/';
